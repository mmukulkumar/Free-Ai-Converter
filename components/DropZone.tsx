        ...
        <div className="flex space-x-3">
          ...
          <Tooltip content="Clear all files from the queue">
            <button
              onClick={onClear}
              disabled={!hasFiles}
              className={`flex items-center px-6 py-2.5 font-semibold text-sm rounded-full transition-all active:scale-95 ${
                  hasFiles 
      ...
      <div
        ...
        className={`
          ...
          ${isDragging 
            ...
            : 'border-slate-200 bg-slate-50 hover:bg-slate-100 hover:border-primary-300'
          }
        `}
      >
        <input
        ...
        <div className="text-center relative z-10 p-6">
          <div className={`w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center transition-colors duration-300 ${isDragging ? 'bg-primary-200 text-primary-700' : 'bg-white shadow-sm text-primary-600'}`}>
            <Upload className="w-8 h-8" />
          </div>
          <h3 className="text-xl font-bold text-slate-700 mb-2">Drop your files here</h3>
          <p className="text-sm text-slate-500">
             or click to browse
          </p>
          <div className="mt-4 inline-flex items-center px-3 py-1 rounded-full bg-white border border-slate-200 text-xs font-medium text-slate-500 shadow-sm">
