...
const ToolSelector: React.FC<ToolSelectorProps> = ({ activeTab, tabs, onTabChange }) => {
    ...
    return (
        <div className="relative w-full border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-900/50 p-4 rounded-t-3xl" ref={dropdownRef}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                    <label className="text-sm font-semibold text-slate-500 dark:text-slate-400">Current Tool:</label>
                </div>

                <div className="relative flex-1 md:max-w-md">
                    <button
                        onClick={() => setIsOpen(!isOpen)}
                        className="w-full flex items-center justify-between px-4 py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl shadow-sm text-slate-700 dark:text-slate-200 font-medium hover:border-primary-300 dark:hover:border-primary-500 hover:ring-2 hover:ring-primary-100 dark:hover:ring-primary-500/20 transition-all active:scale-[0.99] group"
                    >
                        <div className="flex items-center gap-3">
                            <div className={`p-1.5 rounded-lg transition-colors ${isOpen ? 'bg-primary-100 dark:bg-primary-900/40 text-primary-600 dark:text-primary-400' : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 group-hover:text-primary-600 dark:group-hover:text-primary-400 group-hover:bg-primary-50 dark:group-hover:bg-primary-900/20'}`}>
                                <ActiveIcon className="w-4 h-4" />
                            </div>
                            <span className="text-base truncate">{activeLabel}</span>
                        </div>
                        <ChevronDown className={`w-5 h-5 text-slate-400 dark:text-slate-500 transition-transform duration-200 ${isOpen ? 'rotate-180 text-primary-500 dark:text-primary-400' : ''}`} />
                    </button>

                    {/* Dropdown Menu */}
                    {isOpen && (
                        <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-white/10 z-[100] overflow-hidden animate-fade-in-up origin-top ring-1 ring-black/5 dark:ring-white/5">
                            <div className="p-3 border-b border-slate-50 dark:border-white/5 bg-slate-50/30 dark:bg-slate-900/30">
                                <div className="relative">
                                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-500" />
                                    <input
                                        type="text"
                                        placeholder="Search tools..."
                                        className="w-full pl-10 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-lg text-sm focus:border-primary-300 dark:focus:border-primary-500 focus:ring-2 focus:ring-primary-100 dark:focus:ring-primary-500/20 text-slate-700 dark:text-slate-200 outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500"
                                        value={searchTerm}
                                        ...
                                        <div className="px-2">
                                            {groupedTabs[cat].map(tab => (
                                                <button
                                                    ...
                                                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-sm transition-all mb-0.5 ${activeTab === tab.id
