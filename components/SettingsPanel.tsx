...
import { Settings2, ChevronDown, ChevronUp, FileText, Image as ImageIcon, Sliders, Zap, Gauge, RotateCw, Crop, Type, Maximize2 } from 'lucide-react';
import { OptimizerSettings, PdfPageOptions, RasterOptions, ResizeOptions, RotateOptions, CropOptions, WatermarkOptions } from '../types';
...
const SettingsPanel: React.FC<SettingsPanelProps> = ({ activeTab, settings, onChange }) => {
    ...
    // New image tools
    const isResizer = activeTab === 'image-resizer';
    const isRotator = activeTab === 'image-rotator';
    const isCropper = activeTab === 'image-cropper';
    const isWatermark = activeTab === 'image-watermark';
    const isImageTool = isResizer || isRotator || isCropper || isWatermark;
    ...
    // Image tool option handlers
    const updateImageToolOption = (toolKey: string, optionKey: string, value: any) => {
        const currentOptions = settings.imageToolOptions || {};
        const currentToolOptions = (currentOptions as any)[toolKey] || {};
        onChange({
            ...settings,
            imageToolOptions: {
                ...currentOptions,
                [toolKey]: { ...currentToolOptions, [optionKey]: value }
            }
        });
    };

    // Get current image tool values with defaults
    const resizeOpts = settings.imageToolOptions?.resize || { width: 800, height: 600, maintainAspect: true, fit: 'contain' as const };
    const rotateOpts = settings.imageToolOptions?.rotate || { degrees: 90 as const };
    const cropOpts = settings.imageToolOptions?.crop || { x: 0, y: 0, width: 400, height: 400 };
    ...
    return (
        <div className="bg-white border-b border-slate-200 animate-fade-in rounded-xl shadow-sm border mb-6 overflow-hidden">
                        ...
                        {isSvgOutput ? (
                            ...
                            <div className="w-full bg-white px-4 py-3 rounded-xl border border-slate-200 shadow-sm">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded ${settings.rasterOptions.quality > 0.8 ? 'bg-green-100 text-green-700' :
            ...
            {/* --- IMAGE TOOL CONTROLS --- */}
            {isImageTool && (
                <div className="px-6 py-5 bg-gradient-to-r from-blue-50 to-white border-t border-slate-100">
                    ...
                    {isResizer && (
                        <div className="space-y-4">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <div>
                                    <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide">Resize Options</h3>
                                    <p className="text-xs text-slate-500">Set target dimensions for your image</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 uppercase block mb-1">Width (px)</label>
                                    <input
                                        type="number"
                                        value={resizeOpts.width}
                                        onChange={(e) => updateImageToolOption('resize', 'width', parseInt(e.target.value) || 800)}
                                        className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary-400"
                                    />
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 uppercase block mb-1">Height (px)</label>
                                    <input
                                        type="number"
                                        value={resizeOpts.height}
                                        onChange={(e) => updateImageToolOption('resize', 'height', parseInt(e.target.value) || 600)}
                                        className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary-400"
                                    />
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        checked={resizeOpts.maintainAspect}
                                        onChange={(e) => updateImageToolOption('resize', 'maintainAspect', e.target.checked)}
                                ...
                                <select
                                    ...
                                    onChange={(e) => updateImageToolOption('resize', 'fit', e.target.value)}
                    ...
                    {isRotator && (
                        <div className="space-y-4">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <div>
                                    <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide">Rotation</h3>
                                    <p className="text-xs text-slate-500">Select rotation angle</p>
                                </div>
                            </div>
                    ...
                    {isCropper && (
                        <div className="space-y-4">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <div>
                                    <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide">Crop Settings</h3>
                                    <p className="text-xs text-slate-500">Define crop region (from top-left corner)</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 uppercase block mb-1">X Offset</label>
                                    <input type="number" value={cropOpts.x} onChange={(e) => updateImageToolOption('crop', 'x', parseInt(e.target.value) || 0)} className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm" />
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 uppercase block mb-1">Y Offset</label>
                                    <input type="number" value={cropOpts.y} onChange={(e) => updateImageToolOption('crop', 'y', parseInt(e.target.value) || 0)} className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm" />
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 uppercase block mb-1">Width</label>
                                    <input type="number" value={cropOpts.width} onChange={(e) => updateImageToolOption('crop', 'width', parseInt(e.target.value) || 400)} className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm" />
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 uppercase block mb-1">Height</label>
                                    <input type="number" value={cropOpts.height} onChange={(e) => updateImageToolOption('crop', 'height', parseInt(e.target.value) || 400)} className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm" />
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {isWatermark && (
                        <div className="space-y-4">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <div>
                                    <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide">Watermark</h3>
                                    <p className="text-xs text-slate-500">Add text watermark to your image</p>
                                </div>
                            </div>
                            <div className="space-y-4">
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 uppercase block mb-1">Watermark Text</label>
                                    <input
                                        type="text"
                                        value={watermarkOpts.text}
                                        onChange={(e) => updateImageToolOption('watermark', 'text', e.target.value)}
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select
                                            value={watermarkOpts.position}
                                            onChange={(e) => updateImageToolOption('watermark', 'position', e.target.value)}
                                    ...
                                    <div>
                                        <label className="text-xs font-semibold text-slate-500 uppercase block mb-1">Font Size</label>
                                        <input
                                            type="number"
                                            value={watermarkOpts.fontSize}
                                            onChange={(e) => updateImageToolOption('watermark', 'fontSize', parseInt(e.target.value) || 24)}
                                            className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm"
                                        />
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs font-semibold text-slate-500 uppercase block mb-1">Opacity ({Math.round(watermarkOpts.opacity * 100)}%)</label>
                                        <input
                                            type="range"
                                            min="10" max="100" step="10"
                                            value={watermarkOpts.opacity * 100}
                                            onChange={(e) => updateImageToolOption('watermark', 'opacity', parseInt(e.target.value) / 100)}
                                            className="w-full h-2 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-primary-600"
                                        />
                                    </div>
                                    <div>
                                        <label className="text-xs font-semibold text-slate-500 uppercase block mb-1">Color</label>
                                        <input
                                            type="color"
                                            value={watermarkOpts.color}
                                            onChange={(e) => updateImageToolOption('watermark', 'color', e.target.value)}
