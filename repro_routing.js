...
// MOCKED DATA from utils/formats.ts
...
const CORE_TOOLS = [
    ...
    { id: 'png-svg', label: 'PNG to SVG', category: 'vector' },
...
const generateTabs = () => {
    const tabs = [...CORE_TOOLS, ...EXTRA_TOOLS];
    ...
    FORMATS.forEach(fmt => {
        ...
        if (lowerFmt !== 'jpg' && lowerFmt !== 'jpeg') {
            ...
            if (!existingIds.has(id)) {
                tabs.push({ id, label: `${fmt} to JPG`, category: getCategory(fmt) });
                existingIds.add(id);
            }
        }
        ...
        if (lowerFmt !== 'png') {
            ...
            if (!existingIds.has(id)) {
                tabs.push({ id, label: `${fmt} to PNG`, category: getCategory(fmt) });
                existingIds.add(id);
            }
        }
    });
...
const ALL_TABS = generateTabs();
const ALL_IDS = new Set(ALL_TABS.map(t => t.id));
...
console.log('Checking existence of Menu Tools in ALL_TABS...');

let missingCount = 0;
MENU_CATEGORIES.forEach(cat => {
    cat.items.forEach(item => {
        if (!ALL_IDS.has(item.id)) {
            console.log(`❌ Missing ID usage in Menu: ${item.id}`);
            missingCount++;
        }
    });
});
...
if (missingCount === 0) {
    console.log('✅ All menu items allow valid tool usage.');
} else {
    console.log(`found ${missingCount} missing ids`);
}
